...
namespace CliWrap
{
    ...
    public partial class Command
    {
        ...
        private async Task<ExtendedCommandResult> ExecuteAsyncLimited(ProcessEx process, TimeSpan timeLimit, double memoryLimitMb, CancellationToken ct = default)
        {
            ...
            while (!process.HasExited)
            {
                try
                {
                    process._nativeProcess.Refresh();
                    memoryUsed = Math.Max(memoryUsed, process._nativeProcess.PrivateMemorySize64);
                }
                catch
                {
                ...
                try
                {
                    if (memoryUsed > memoryLimitMb * 1024 * 1024)
                    {
                        process.Kill();
                        break;
                    }
            ...
            return new ExtendedCommandResult(
                process.ExitCode,
                process.StartTime,
                process.ExitTime,
                memoryUsed / (1024.0 * 1024)
            );
        }
        ...
        public CommandTask<CommandResult> ExecuteAsync(CancellationToken cancellationToken = default)
        {
            ...
            return new CommandTask<CommandResult>(task, process);
        }
        /// <summary>
        /// Executes the process with resource limits
        /// </summary>
        /// <param name="memoryLimit"></param>
        /// <param name="timeLimit"></param>
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        public CommandTask<ExtendedCommandResult> ExecuteLimitedAsync(double memoryLimit, TimeSpan timeLimit, CancellationToken cancellationToken = default)
        {
            var process = new ProcessEx(GetStartInfo());
            var task = ExecuteAsyncLimited(process, timeLimit, memoryLimit, cancellationToken);
            return new CommandTask<ExtendedCommandResult>(task, process);
        }
